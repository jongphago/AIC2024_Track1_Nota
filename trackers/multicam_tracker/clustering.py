...
class Clustering:
    def __init__(
        self, appearance_thresh=0.25, euc_thresh=0.2, match_thresh=0.8, map_size=None
    ):
    ...
    def update(self, trackers, cur_frame, scene):
        ...
        for a_tracker, b_tracker in tracker_pairs:
            ...
            if 0 not in emb_dists.shape:
                ...
                for i in range(len(dists)):
                    for j in range(len(dists[0])):
                        ...
                        dists[i][j] += (1 - ratio) * norm_euc_dists[i][
                            j
                        ] + ratio * norm_emb_dists[i][j]
            ...
            for id_atrack, id_btrack in matches:
                a_global_id = a_tracker.tracked_stracks[
                    a_match_to_id[id_atrack]
                ].t_global_id
                b_global_id = b_tracker.tracked_stracks[
                    b_match_to_id[id_btrack]
                ].t_global_id
                matched_id = a_global_id if a_global_id <= b_global_id else b_global_id
                total_global_ids = [
                    track.t_global_id for track in a_tracker.tracked_stracks
                ] + [track.t_global_id for track in b_tracker.tracked_stracks]
                total_global_ids_lost = [
                    track.t_global_id for track in a_tracker.lost_stracks
                ] + [track.t_global_id for track in b_tracker.lost_stracks]
                if total_global_ids.count(
                    matched_id
                ) > 1 or total_global_ids_lost.count(matched_id):
                    continue

                if (
                    a_tracker.tracked_stracks[a_match_to_id[id_atrack]].matched_dist
                    and a_global_id > b_global_id
                ):
                    cur_dist = (
                        euc_dists[id_atrack][id_btrack]
                        * emb_dists[id_atrack][id_btrack]
                    )
                    # cur_dist = dists[id_atrack][id_btrack]
                    old_dist = a_tracker.tracked_stracks[
                        a_match_to_id[id_atrack]
                    ].matched_dist
                    if cur_dist > old_dist:
                        continue
                elif (
                    b_tracker.tracked_stracks[b_match_to_id[id_btrack]].matched_dist
                    and a_global_id < b_global_id
                ):
                    cur_dist = (
                        euc_dists[id_atrack][id_btrack]
                    ...
                    # cur_dist = dists[id_atrack][id_btrack]
                    old_dist = b_tracker.tracked_stracks[
                        b_match_to_id[id_btrack]
                    ].matched_dist
                    if cur_dist > old_dist:
                        continue

                a_tracker.tracked_stracks[a_match_to_id[id_atrack]].t_global_id = (
                    b_tracker.tracked_stracks[b_match_to_id[id_btrack]].t_global_id
                ) = matched_id
    ...
    def pose_check_all(self, pose_):
        ...
        return (
            np.array([head, neck, left_shoulder, right_shoulder]) > 0.5,
            np.array(
                [
                    left_elbow,
                    right_elbow,
            ...
            > 0.5,
            np.array([shoulder, elbow, wrist, hip, knee, ankle]) > 0.5,
        )
    ...
    def update_using_mctracker(self, trackers, mc_tracker):
        ...
        for tracker in trackers:
            ...
            mct_features = [
                feat for track in mtrack_pools for feat in list(track.features)
            ]
            ...
            if 0 in shape:
                ...
                emb_dists = (
                    matching.embedding_distance(sct_features, mct_features) / 2.0
                )
                emb_dists = group_dists(
                    emb_dists,
                    [1 for _ in range(len(sct_features))],
                    length_mcts,
                    shape,
                    normalize=False,
                )
                euc_dists = (
                    matching.euclidean_distance(sct_locations, mct_centroids)
                    / self.max_len
                )
            ...
            euc_dists = (
                matching.euclidean_distance(sct_locations, mct_centroids) / self.max_len
            )
            dists = euc_dists
    ...
    for i, len_e in enumerate(lengths_exists):
        for j, len_n in enumerate(lengths_new):
            ...
            if shape == (1, 1):
                ...
                emb_dists[i, j] = (
                    np.sum(rerank_dists[start_x:end_x, start_y:end_y])
                    / total_sum
                    / (len_e * len_n)
                )
