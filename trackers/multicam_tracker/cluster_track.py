    ...
    def update_features(self, features, poses, img_paths, tlbrs, coords):
        ...
        for feat, pose, img_path, tlbr in zip(features, poses, img_paths, tlbrs):
            ...
            if num_point >= self.pose_thresh:
                """Procrustes Analysis"""
                if img_path in self.path_tlbr or self.compare_poses(pose):
                    continue
...
class MCTracker:
    ...
    def update(self, trackers, groups, scene=None):
        ...
        if len(
            groups
        ):  # group type: array[[t_groud_id, features, centroid(location)], ...]
            global_ids = groups[:, 0]
            features = groups[:, 1]
            centroids = groups[:, 2]
            poses = groups[:, 3]
        ...
        if len(centroids) > 0:
            new_groups = [
                MTrack(g, c, f, p, ph, t, cd, self.min_hits, scene)
                for (g, c, f, p, ph, t, cd) in zip(
                    global_ids, centroids, features, poses, paths, tlbrs, coords
                )
            ]
        else:
            new_groups = []
        ...
        if 0 in shape:
            ...
            if self.frame_id % 10 == 0:
                rerank_dists = (
                    matching.embedding_distance(exist_features, new_features) / 2.0
                )
                emb_dists = grouping_rerank(
                    rerank_dists, lengths_exists, lengths_new, shape, normalize=False
                )
                dists = emb_dists
            else:
                rerank_dists = (
                ...
                euc_dists = (
                    matching.euclidean_distance(exist_centroids, new_centroids)
                    / self.max_len
                )
        ...
        for it in u_exist:
            ...
            if not track.state == TrackState.Lost and (
                not track.state == TrackState.Removed
            ):
                track.mark_lost()
        ...
        if 0 in shape:
            ...
            rerank_dists = (
                matching.embedding_distance(lost_features, new_features) / 2.0
            )
            emb_dists = grouping_rerank(
                rerank_dists, lengths_lost, lengths_new, shape, normalize=False
            )

        dists = emb_dists

        matches, u_lost, u_new = matching.linear_assignment(
            dists, thresh=self.emb_thresh
        )
        ...
        """ Merge """
        self.tracked_mtracks = [
            t for t in self.tracked_mtracks if t.state == TrackState.Tracked
        ]
        ...
        # self.tracked_mtracks, self.lost_mtracks = remove_duplicate_mtracks(
        # self.tracked_mtracks, self.lost_mtracks
        # )

        output_mtracks = [track for track in self.tracked_mtracks if track.is_activated]
        unconfirmed_mtracks = [
            track for track in self.tracked_mtracks if not track.is_activated
        ]
        print(f"tracking ids: {[m.track_id for m in output_mtracks]}")
        print(
            f"unconfirmed_tracks ids: \
                {[[m.track_id, m.tracklet_len]for m in unconfirmed_mtracks]}"
        )
    ...
    def refinement_clusters(self):
        tracked_mtracks = [
            track for track in self.tracked_mtracks if track.is_activated
        ]
        unconfirmed_mtracks = [
            track for track in self.tracked_mtracks if not track.is_activated
        ]
        ...
        for mtrack in mtracks:
            ...
            a_max_area_feat = self.get_max_area_feature(
                a_cluster_features, a_path_keys, path_tlbr
            )
            b_max_area_feat = self.get_max_area_feature(
                b_cluster_features, b_path_keys, path_tlbr
            )

            emb_dist = (
                matching.embedding_distance(a_max_area_feat, b_max_area_feat) / 2.0
            )
            ...
            if emb_dist > self.refine_1st_emb:
                print(
                    f"refine step1: cluster {mtrack.track_id} is consist of two people ({sum(labels == 0)} + {sum(labels == 1)}) / dist: {emb_dist:.4f}"
                )
        ...
        for a_cluster, b_cluster in cluster_pairs:
            if (
                a_cluster.state == TrackState.Removed
                or b_cluster.state == TrackState.Removed
            ):
                continue
            a_cluster_features = list(a_cluster.features)
            b_cluster_features = list(b_cluster.features)
            emb_dists = (
                matching.embedding_distance(a_cluster_features, b_cluster_features)
                / 2.0
            )
            ...
            euc_dist = matching.euclidean_distance(
                a_cluster_centroid, b_cluster_centroid
            )[0][0]
            ...
            if emb_dist < self.refine_2nd_emb and euc_dist < self.refine_2nd_euc:
                if (
                    a_cluster.start_frame == 1
                    and b_cluster.start_frame == 1
                    and self.frame_id > 10
                ):
                    continue
                print(
                    f"refine step2: cluster {a_cluster.track_id} and cluster {b_cluster.track_id} are same person"
                )
        ...
        self.tracked_mtracks = [
            t for t in tracked_mtracks if t.state == TrackState.Tracked
        ]
        ...
        for tracked_cluster in tracked_mtracks:
            if tracked_cluster.state == TrackState.Removed:
                continue
            for lost_cluster in lost_mtracks:
                if lost_cluster.state == TrackState.Tracked:
                    continue
                ...
                emb_dists = (
                    matching.embedding_distance(
                        tracked_cluster_feats, lost_cluster_feats
                    )
                    / 2.0
                )
                emb_dist = np.mean(emb_dists)
                ...
                if (
                    emb_dist < self.refine_3rd_emb
                    and lost_cluster.track_id < tracked_cluster.track_id
                    and tracked_cluster.start_frame != 1
                ):
                    print(
                        f"refine step3: cluster {tracked_cluster.track_id} is removed (matched with lost cluster {lost_cluster.track_id})"
                    )
                    lost_cluster.re_activate(
                        tracked_cluster, self.frame_id, new_id=False
                    )
